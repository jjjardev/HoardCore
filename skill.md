...
9. **Parallel ingest is opt-in via `--parallel`/config** — threaded ingest engages only for batches of **8+ chunks**. Force it per-run with `--parallel` (or force off with `--no-parallel`); default follows `indexer.parallel` in `hoardcore.toml` (on). On small batches it's a silent no-op (the sequential path is used).
...
- **Release gates (CI enforces these)**: `tools/check_version.py` must report OK (`__version__` == `pyproject.toml` version, and == the git tag on `v*` pushes) and coverage must sit at ≥70% (`--cov-fail-under=70`) or CI fails. Run `venv/bin/python tools/check_version.py` and `make coverage` before tagging a release, exactly as the CI check job does.
